...
class Solution:
    ...
    def checkValid(self):
        ...
        for col in range(n):
            col_vals = [] 
            for row in range(n):
                col_vals.append(self.mat[row][col])
...
#             for col in range(n):
#                 val = matrix[row][col]
#                 if val in row_seen[row] or val in col_seen[col]:
#                     return False
